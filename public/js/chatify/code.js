...
/**
 *-------------------------------------------------------------
 * Pusher channelChats and event listening..
 *-------------------------------------------------------------
 */

// subscribe to the channelChat
var channelChat = pusher.subscribe('private-chatify')

// Listen to messages, and append if data received
channelChat.bind('messaging', function (data) {
...
channelChat.bind('client-typing', function (data) {
    if (data.from_id == messenger.split('_')[1] && data.to_id == auth_id) {
...
channelChat.bind('client-seen', function (data) {
    if (data.from_id == messenger.split('_')[1] && data.to_id == auth_id) {
        if (data.seen == true) {
...
channelChat.bind('client-contactItem', function (data) {
...
/*
// -------------------------------------
// presence channelChat [User Active Status]
...
function isTyping (status) {
    return channelChat.trigger('client-typing', {
        from_id: auth_id, // Me
...
function makeSeen (status) {
    ...
    */
    ...
    return channelChat.trigger('client-seen', {
        from_id: auth_id, // Me
        to_id: messenger.split('_')[1], // Messenger
        seen: status,
    })
...
function sendContactItemUpdates (status) {
    return channelChat.trigger('client-contactItem', {
...
$(document).ready(function () {
    ...
    // check if pusher has access to the channelChat [Internet status]
    ...
    // Image modal
    $('body').on('click', '.chatbox-image', function () {
