...
@if($get == 'saved')
	<table
		class="messenger-list-item m-li-divider @if('user_'.Auth::user()->id == $id && $id != "0") m-list-active @endif">
		<tr data-action="0">
			{{-- Avatar side --}}
			<td>
		...
		<tr data-action="0">
			...
			<td>
				<p data-id="{{ $type.'_'.$user->id }}">
					{{ strlen($user->full_name) > 20 ? trim(substr($user->full_name,0,20)).'..' : $user->full_name }}
